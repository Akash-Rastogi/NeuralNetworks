...
from pandas.tools.rplot import Layer
...
class ClNeuralNetwork:
    ...
    def adjust_weights(self, input_samples, error):
        for layer_index, layer in enumerate(self.layers):
            errorMatrix = np.matrix(error)
            inputMatrixTransposed = np.transpose(np.matrix(input_samples))
            
            numOfColumns = len(inputMatrixTransposed)
            bias = np.ones((numOfColumns, 1))
            
            inputMatrixWithBias = np.c_[inputMatrixTransposed, bias]
    
            product =  errorMatrix * inputMatrixWithBias
